...
const getAuthTokenPF = async () => {
  const baseUrl = process.env.SSO_URL;

  if (!process.env.CLIENT_ID || !process.env.CLIENT_SECRET || !process.env.CPF || !process.env.PASSWORD || !baseUrl) {
    throw new Error('Uma ou mais variáveis de ambiente de autenticação não foram encontradas no arquivo .env');
  }
  ...
  const credentialsPF = {
    ...
    client_id: process.env.CLIENT_ID.trim(),
    client_secret: process.env.CLIENT_SECRET.trim(),
    username: process.env.CPF.trim(),
    password: process.env.PASSWORD.trim(),
  };

  const requestBodyPF = new URLSearchParams(credentialsPF).toString();
  ...
  try {
    ...
    if (response.status !== 200 || !response.body.access_token) {
      console.error('Resposta da API de autenticação PF:', response.body);
      throw new Error('Falha ao obter o access_token como transportador PF. Verifique as credenciais no arquivo .env.');
    }
...
module.exports = { getAuthToken, getAuthTokenPF, getAuthTokenSimulacao };